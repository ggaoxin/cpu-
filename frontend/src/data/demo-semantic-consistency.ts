...
export function demoApiPayloadForTool(toolId: string): AnyRecord | undefined {
  const payloadsByTool: Record<string, AnyRecord> = {
    ...
    'zh-classify': { input_type: 'text', document_title: zhPapers[1].title, chinese_scientific_document_text: zhPapers[1].abstract },
    'en-classify': { input_type: 'text', document_title: enClassificationPapers[0].title, english_scientific_document_text: enClassificationPapers[0].abstract },
    'domain-classify': { input_type: 'text', document_title: zhPapers[2].title, domain_scientific_literature_data: [{ id: 'TEXT001', title: zhPapers[2].title, text: zhPapers[2].abstract }], professional_domain: '生物医学信息学' },
    'zh-keyword': { input_type: 'text', document_title: zhPapers[1].title, abstract: zhPapers[1].abstract, min_keywords: 3, max_keywords: 8, preserve_order: true },
    'en-keyword': { input_type: 'text', document_title: enClassificationPapers[0].title, abstract: enClassificationPapers[0].abstract, min_keywords: 3, max_keywords: 8, normalize_terms: true, preserve_order: true },
    'rq-detect': { input_type: 'text', document_title: zhPapers[1].title, text: zhPapers[1].abstract, text_format: 'auto', document_scope: 'full_document' },
    'citation-sentiment': { input_type: 'text', citation_sentence: citationProfiles[0].sentence, previous_context: citationProfiles[0].previous, next_context: citationProfiles[0].next, citation_metadata: [{ reference_id: 'REF-1', citation_marker: citationProfiles[0].marker, authors: citationProfiles[0].authors, work_name: citationProfiles[0].work }] },
    'citation-intent': { input_type: 'text', citation_sentence: citationProfiles[0].sentence, previous_context: citationProfiles[0].previous, next_context: citationProfiles[0].next, citation_metadata: [{ reference_id: 'REF-1', citation_marker: citationProfiles[0].marker, authors: citationProfiles[0].authors, work_name: citationProfiles[0].work }] },
    'definition-detect': { text: '智慧治理是以数据要素汇聚、智能分析和协同决策为基础，对高校体育场馆资源、服务与运营实施精细化管理的治理方式。数据驱动决策是利用场馆运行、用户行为和资源配置数据支持管理判断与方案优化的决策过程。', domain: 'sports_science', return_mapping: true, return_position: true },
    'general-ner': { text: '刘亭亭、吕大刚和李鸿晶使用美国太平洋地震工程研究中心PEER数据库开展地震动记录研究。', language: 'zh', entity_types: ['PERSON', 'ORGANIZATION', 'LOCATION', 'EVENT'], normalize: true },
    'research-ner': { text: pureSingleTextByTool['research-ner'], language: 'zh', entity_types: ['METHOD', 'DATASET', 'THEORY', 'RESEARCH_PROBLEM'], return_context: true },
    'domain-ner': { text: pureSingleTextByTool['domain-ner'], domain: 'medicine', ontology: 'biomedical_ontology', return_kb_id: true },
