...
import { buildCallCode, modesFor, pretty, requestParameterRowsFor, responseFor } from '../utils/tooling'
...
async function copyWithToast(value: string, name: string) { try { await copyText(value); copied.value = name; setTimeout(() => copied.value = '', 1400) } catch { copied.value = '复制失败'; setTimeout(() => copied.value = '', 1400) } }
</script>
