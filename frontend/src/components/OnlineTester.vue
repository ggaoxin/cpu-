              ...
              <div class="two-column deep-cluster-metadata-grid deep-cluster-anchor-grid">
                ...
                <div class="field"><label><span class="label-main">人工标注类目标签数据</span><small>可选</small></label>
                  <div class="requirement-resource-controls">
                    ...
                    <div v-else class="resource-upload-wrap"><label class="resource-upload-zone"><input type="file" accept=".json" @change="handleAnchorFileChange('manually_labeled_category_data', $event)" /><span>⇧</span><b>{{ anchorGoldFile?.name || '点击上传人工标注类目标签数据' }}</b><small>仅标准 JSON 数组生效</small></label><div class="anchor-upload-actions"><button type="button" class="primary-btn" :disabled="anchorUploadingKey === 'manually_labeled_category_data' || !anchorGoldFile" @click="submitAnchorResource('manually_labeled_category_data')">{{ anchorUploadingKey === 'manually_labeled_category_data' ? '提交中…' : '提交' }}</button><button v-if="anchorGoldFile" class="hover-copy-btn resource-cancel-btn" type="button" @click="clearAnchorFile('manually_labeled_category_data')">✕ 取消</button></div></div><p class="anchor-format-hint">格式要求：JSON 数组，每条含 document_id、title、abstract 与类目标注字段 technical_cluster_id（应用轴聚类用 application_cluster_id），可用 cluster_name 指定类目显示名。结构不符的文件可上传成功，但自定义类目不会生效，聚类将回退内置类目体系。</p>
                  </div>
                </div>
              </div>
            </div>
