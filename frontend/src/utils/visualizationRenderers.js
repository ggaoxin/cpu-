...
const nerRecordCell = (rows, index, record, label, recordKey = '__record') => {
  if (index > 0 && rows[index - 1]?.[recordKey] === record) return ''
  let rowspan = 1
  while (rows[index + rowspan]?.[recordKey] === record) rowspan += 1
  // rowspan=组内主行数（全部收起时的正确值）；展开"查看详情"行时由
  // VisualizationModal 的切换处理器同步 +1，收起 -1——行数不匹配会溢出串组
  return `<td class="viz-merged-source-cell" rowspan="${rowspan}">${renderTextWithMath(label)}</td>`
}

function mergedRecordCell(rows, index, record, label, recordKey = 'record', rowMult = 1) {
  if (index > 0 && rows[index - 1]?.[recordKey] === record) return ''
  let rowspan = 1
  while (rows[index + rowspan]?.[recordKey] === record) rowspan += 1
  // rowMult：每个数据行在 DOM 里占的实际行数（NER 表每实体带一行"查看详情"
  // 展开行 → 2；rowspan 不含详情行会错位串到文献列，2026-09-11 修复）
  return `<td class="viz-merged-source-cell" rowspan="${rowspan * rowMult}">${renderTextWithMath(label)}</td>`
}
...
const summaryCards = (items, gridClass, cardClass = 'distribution-summary-item', valueClass = 'distribution-summary-value', labelClass = 'distribution-summary-label', style = '') => `
  <div class="${gridClass}"${style ? ` style="${style}"` : ''}>${items.map(([label, value]) => `<div class="${cardClass}"><div class="${valueClass}">${escapeHtml(value)}</div><div class="${labelClass}">${escapeHtml(label)}</div></div>`).join('')}</div>`
...
// 基金语步内容高亮：语步文本是 LLM 归纳内容，其中能在原文（document.abstract，
// 文件模式为全文）找到的片段（原描述/原句）用黄色标出。最长贪心匹配：从当前位置
// 取最长可命中子串（≥4 字，含数字/英文术语 3 字起，去空白对比），命中段高亮、
// 未命中段推进到下一标点，避免碎片高亮。
function renderFundMoveContent(item, record) {
  const text = item.text || item.sentence || ''
  if (!text) return '—'
  const source = String(record.payload?.document?.abstract
    || record.payload?.input?.text || '')
  if (!source) return renderTextWithMath(text)
  const flat = value => String(value).replace(/\s+/g, '')
  const srcFlat = flat(source)
  if (!srcFlat) return renderTextWithMath(text)
  // 片段最小长度：默认 4 字；含数字或英文术语（更独特、误报低）降到 3 字
  const minLenOf = seg => (/\d|[A-Za-z]/.test(seg) ? 3 : 4)
  let i = 0
  let html = ''
  while (i < text.length) {
    let matched = 0
    for (let len = text.length - i; len >= 3; len--) {
      const seg = flat(text.slice(i, i + len))
      if (seg.length >= minLenOf(seg) && srcFlat.includes(seg)) { matched = len; break }
    }
    if (matched) {
      html += `<mark class="fund-move-orig-mark-v663">${renderTextWithMath(text.slice(i, i + matched))}</mark>`
      i += matched
    } else {
      // 未命中：推进到下一个句读标点，整段作为归纳内容（不高亮）
      const rest = text.slice(i)
      const m = rest.search(/[。！？；，、：]/)
      const step = m >= 0 ? m + 1 : rest.length
      html += renderTextWithMath(text.slice(i, i + step))
      i += step
    }
  }
  return html
...
function renderFundMove(response) {
  ...
  // 统计有文本位置来源的语步数（fundMoveSourceGroups 非空即该语步有可溯源来源）。
  ...
  const resultRows = records.flatMap(record => {
    ...
    return moves.map((item, index) => `<tr class="${index === 0 ? 'fund-move-project-start-v663 ' : ''}fund-move-project-${record.index % 2 === 0 ? 'odd' : 'even'}-v663">${index === 0 ? `<td class="fund-move-project-name-v663" rowspan="${moves.length}">${escapeHtml(projectNameLabel(record))}</td>` : ''}<td><span class="fund-move-label-badge-v663">${escapeHtml(item.label || '—')}</span></td><td class="fund-move-src-cell-v663">${(item.start != null && item.end != null && String(record.payload?.input_type || record.payload?.document?.input_type || 'text').includes('text') && !String(record.payload?.input_type || '').includes('file')) ? `字符 ${item.start}-${item.end}` : renderFundMoveSource(item)}</td><td>${renderFundMoveContent(item, record)}</td><td>${confidence(item.confidence)}</td></tr>`)
  }).join('')
...
// 位置列显示字符范围（start/end 定位），题名取 document.title。
function renderAbstractMove(response, english = false) {
  const records = recordsOf(response)
  const rows = records.flatMap(record => array(record.payload.moves).map(move => ({ ...move, record })))
  const present = new Set(rows.map(item => item.label).filter(Boolean)).size
  // 后端 moves 不含句序号（字段为 move_code/move_name/label/text/confidence/start/end），
  // 摘要总句数取 data 顶层 sentence_count（后端统计字段）。
...
function terminologyLabel(value) {
  ...
  if (source.type === 'external') return '领域术语库'
...
function renderResearchQuestions(response) {
  ...
  return `<div class="rq-result-root" data-viz-group>
    ...
    <div class="rq-tabs"><button class="rq-tab-btn active" data-viz-tab="sentences">研究问题句</button><button class="rq-tab-btn" data-viz-tab="phrases">研究问题短语</button><button class="rq-tab-btn" data-viz-tab="structured">结构化研究问题</button><button class="rq-tab-btn" data-viz-tab="statistics">研究问题统计摘要</button></div>
...
function renderCitation(response, intent = false) {
  ...
  return `<div class="${prefix}-result-root" data-viz-group>
    ...
    <div class="${prefix}-tab-panel" data-viz-panel="context" hidden><div class="${prefix}-context-list">${(() => { const _docNames = new Set(items.map(item => item.__record?.name).filter(Boolean)); const _multiDoc = _docNames.size > 1; const _fileBadge = item => { const _n = item.__record?.name; return _multiDoc && _n ? `<span style="display:inline-block;max-width:220px;overflow:hidden;text-overflow:ellipsis;vertical-align:bottom;white-space:nowrap;padding:1px 8px;margin-right:6px;border-radius:10px;background:#eef2f7;color:#3d5a80;font-size:12px;font-weight:500" title="${escapeHtml(_n)}">${escapeHtml(_n)}</span>` : '' }; return items.map((item, index) => { const ctx = object(item.context); return`<article class="${prefix}-context-card"><h4>${_fileBadge(item)}${escapeHtml(valueOf(item, ['citation_id'], `引用 ${index + 1}`))} · ${escapeHtml(valueOf(item, intent ? ['intent'] : ['sentiment']))}</h4><dl><dt>前文</dt><dd>${renderTextWithMath(valueOf(ctx, ['previous_sentence', 'before']))}</dd><dt>引用句</dt><dd class="current">${renderTextWithMath(valueOf(ctx, ['current_sentence'], item.citation_sentence || '—'))}</dd><dt>后文</dt><dd>${renderTextWithMath(valueOf(ctx, ['next_sentence', 'after']))}</dd></dl></article>` }).join('') || `<div class="${prefix}-context-empty">暂无上下文片段。</div>` })()}</div></div>
  </div>`
}
...
function renderDefinitions(response) {
  ...
  const reportConclusion = `本次共识别 ${definitionCount} 条概念定义句，提取 ${conceptCount} 个概念词并形成 ${mappingCount} 组结构化映射，平均置信度为 ${fixed(averageConfidence)}。${definitionDistributionTitle}各项统计均依据当前响应结果汇总。`
  return `<div class="definition-result-root" data-viz-group>
    ...
    <div class="definition-tab-panel" data-viz-panel="statistics" hidden><article class="definition-analysis-report"><header class="definition-report-cover"><div><h3>识别统计分析报告</h3><p>汇总当前概念定义识别的任务概况、识别质量与来源章节分布。</p></div></header><section class="definition-report-section"><h4><i>一</i>任务概况</h4><div class="definition-report-summary"><span><b>${definitionCount}</b><small>识别定义句</small></span><span><b>${conceptCount}</b><small>提取概念词</small></span><span><b>${mappingCount}</b><small>结构化映射</small></span></div></section><div class="definition-report-columns"><section class="definition-report-section"><h4><i>二</i>识别质量分析</h4><dl class="definition-report-definition"><div><dt>平均置信度</dt><dd>${fixed(averageConfidence)}</dd></div><div><dt>概念映射覆盖率</dt><dd>${fixed(mappingCoverage, 1)}%</dd></div><div><dt>${coverageLabel}</dt><dd>${sectionRows.length}</dd></div></dl></section><section class="definition-report-section"><h4><i>三</i>${definitionDistributionTitle}</h4><div class="distribution-table-wrap"><table class="distribution-table definition-report-table"><thead><tr>${records.length > 1 ? '<th>编号</th>' : ''}<th>${definitionSourceHeading}</th><th>定义句数量</th><th>占比</th></tr></thead><tbody>${sectionRows.map(item => `<tr>${records.length > 1 ? `<td>${escapeHtml((item.ids || []).join('、') || '—')}</td>` : ''}<td>${escapeHtml(item.section || item.name || '未返回位置')}</td><td>${number(item.count)}</td><td>${fixed(item.percentage != null ? item.percentage : number(item.ratio) * 100, 1)}%</td></tr>`).join('') || `<tr><td colspan="${records.length > 1 ? 4 : 3}">暂无位置分布统计。</td></tr>`}</tbody></table></div></section></div><section class="definition-report-section definition-report-conclusion"><h4><i>四</i>分析结论</h4><p>${renderTextWithMath(reportConclusion)}</p></section></article></div>
  </div>`
}
...
function renderNer(response, variant) {
  ...
  // domain-ner 领域统计卡跟随实际领域标签（2026-09-16 用户定调）：原硬编码
  // 医学/化工/物理——用户上传本体后领域是自定义的（如"临床医学"），三卡全 0；
  // 改为按实体实际出现的领域动态生成（≤4 个防溢出），无领域标签时回落默认三卡
  const domainCards = (() => {
    const counts = {}
    entities.forEach(item => {
      const d = String(item.domain_name || item.domain || '').trim()
      if (d && d !== '—') counts[d] = (counts[d] || 0) + 1
    })
    const cards = Object.entries(counts).slice(0, 4)
    return cards.length ? cards : [['医学', 0], ['化工', 0], ['物理', 0]]
  })()
  const cards = general
    ...
    : research
      ...
      : [['专业实体总数', entities.length], ...domainCards, ['已映射', entities.filter(item => item.standard_kb_id).length]]
  ...
  // 映射状态区分来源（2026-09-16 用户定调，对齐 citation 的"内置向量映射 vs
  // 已映射（用户映射规则命中）"）：用户词表确定性命中 ≠ LLM 现场拟词
  const mappingStatusLabel = item => {
    if (item.mapping_status === '未映射') return '未映射'
    return item.mapping_source === '用户标准词表' ? '已映射（用户上传资源命中）' : '内置映射'
  }
  ...
  if (general) {
    ...
    return `<div class="ner-result-root" data-viz-group>
      ...
      <div class="ner-tab-panel"><div class="distribution-table-wrap"><table class="distribution-table ner-results-table"><colgroup><col style="width:12%"><col style="width:10%"><col style="width:13%"><col style="width:10%"><col style="width:13%"><col style="width:36%"><col style="width:6%"></colgroup><thead><tr><th>文献</th><th>编号</th><th>实体名称</th><th>实体类别</th><th>实体位置</th><th>语境片段</th><th>置信度</th></tr></thead><tbody>${entities.map((item, index) => `<tr class="ner-main-row">${nerRecordCell(entities, index, item.__record, item.__record?.name || valueOf(item, ['file_name'], '当前结果'))}<td><span class="distribution-level-tag">${escapeHtml(idValue(item, index))}</span></td><td><div class="ner-entity-text">${renderTextWithMath(textValue(item))}</div></td><td><span class="ner-type-badge">${escapeHtml(generalTypeLabel(item))}</span></td><td>${escapeHtml(nerPosition(item))}</td><td>${renderTextWithMath(item.context || '—')}</td><td>${confidence(item.confidence)}</td></tr>`).join('') || '<tr><td colspan="7">未识别到实体。</td></tr>'}</tbody></table></div></div>
    </div>`
  }
  if (!research) {
    const domainHeaders = ['文献', '编号', '专业实体', '领域标签', '实体类型', '位置', '标准知识库映射ID', '置信度', '操作']
    const domainEntityRows = entities.map((item, index) => `<tr class="domain-research-main-row">${nerRecordCell(entities, index, item.__record, item.__record?.name || valueOf(item, ['file_name'], '当前结果'))}<td><span class="distribution-level-tag">${escapeHtml(idValue(item, index))}</span></td><td><div class="domain-research-entity-text">${renderTextWithMath(textValue(item))}</div></td><td><span class="domain-research-domain-badge">${escapeHtml(item.domain_name || item.domain || '—')}</span></td><td><span class="domain-research-type-badge">${escapeHtml(typeValue(item))}</span></td><td style="white-space:nowrap">${escapeHtml(nerPosition(item))}</td><td><span class="domain-kb-id">${escapeHtml(item.standard_kb_id || '内置知识库')}</span><br><span class="domain-mapping-status">${escapeHtml(item.mapping_status || '已映射')}</span></td><td>${confidence(item.confidence)}</td><td><button class="domain-research-detail-btn" data-viz-detail="domain-research-detail-${index}">查看详情</button></td></tr><tr id="domain-research-detail-${index}" class="domain-research-detail-row" hidden><td class="domain-research-detail-cell" colspan="8"><div class="domain-research-detail-content"><div class="domain-research-detail-label">专业实体</div><div class="domain-research-detail-value">${renderTextWithMath(textValue(item))}</div><div class="domain-research-detail-label">领域标签</div><div class="domain-research-detail-value">${escapeHtml(item.domain_name || item.domain || '—')}</div><div class="domain-research-detail-label">实体类型</div><div class="domain-research-detail-value">${escapeHtml(typeValue(item))}</div><div class="domain-research-detail-label">位置</div><div class="domain-research-detail-value">${escapeHtml(nerPosition(item))}</div><div class="domain-research-detail-label">关联上下文</div><div class="domain-research-detail-value">${renderTextWithMath(item.context || '—')}</div><div class="domain-research-detail-label">标准知识库映射ID</div><div class="domain-research-detail-value">${escapeHtml(item.standard_kb_id || '未映射')}</div></div></td></tr>`).join('')
    ...
    const _summaryStyle = `grid-template-columns:repeat(${Math.min(Math.max(cards.length, 3), 5)},minmax(0,1fr))`
    return `<div class="domain-research-result-root" data-viz-group>
      ${summaryCards(cards, summaryClass, undefined, undefined, undefined, _summaryStyle)}
      ...
      <div class="domain-research-tab-panel" data-viz-panel="entities"><div class="distribution-table-wrap"><table class="distribution-table domain-research-results-table"><colgroup><col style="width:10%"><col style="width:5%"><col style="width:14%"><col style="width:9%"><col style="width:12%"><col style="width:8%"><col style="width:14%"><col style="width:7%"><col style="width:7%"></colgroup><thead><tr>${domainHeaders.map(item => `<th>${item}</th>`).join('')}</tr></thead><tbody>${domainEntityRows || '<tr><td colspan="9">未识别到达到阈值的实体。</td></tr>'}</tbody></table></div></div>
      <div class="domain-research-tab-panel" data-viz-panel="mapping" hidden><div class="distribution-table-wrap"><table class="distribution-table domain-research-mapping-table"><thead><tr><th>编号</th><th>当前识别实体</th><th>领域标签</th><th>实体类型</th><th>映射状态/置信度</th></tr></thead><tbody>${domainMappingRows || '<tr><td colspan="5">暂无映射结果。</td></tr>'}</tbody></table></div></div>
    </div>`
  }
  const headers = general
    ? ['文献', '编号', '实体名称', '实体类别', '语言', '来源位置', '置信度', '操作']
    : research
      ? ['文献', '编号', '科研实体', '实体类型', '句子位置', '映射标准词', '置信度', '操作']
      : ['文献', '编号', '专业实体', '领域标签', '实体类型', '句子位置', '知识库ID', '置信度', '操作']
  return `<div class="${prefix}-result-root" data-viz-group>
    ...
    <div class="${tabClass}-tab-panel" data-viz-panel="entities"><div class="distribution-table-wrap"><table class="distribution-table ${prefix}-results-table">${research ? '<colgroup><col style="width:18%"><col style="width:7%"><col style="width:21%"><col style="width:10%"><col style="width:14%"><col style="width:14%"><col style="width:7%"><col style="width:9%"></colgroup>' : ''}<thead><tr>${headers.map(item => `<th>${item}</th>`).join('')}</tr></thead><tbody>${entities.map((item, index) => `<tr class="${prefix}-main-row">${nerRecordCell(entities, index, item.__record, item.__record?.name || valueOf(item, ['file_name'], '当前结果'))}<td><span class="distribution-level-tag">${escapeHtml(idValue(item, index))}</span></td><td><div class="${prefix}-entity-text">${renderTextWithMath(textValue(item))}</div></td>${general ? `<td><span class="ner-type-badge">${escapeHtml(typeValue(item))}</span></td><td>${escapeHtml(item.language || '—')}</td><td>${escapeHtml(nerPosition(item))}</td>` : research ? `<td><span class="research-ner-type-badge">${escapeHtml(typeValue(item))}</span></td><td>${escapeHtml(nerPosition(item))}</td><td>${escapeHtml(mappedValue(item))}</td>` : `<td><span class="domain-research-domain-badge">${escapeHtml(item.domain_name || item.domain || '—')}</span></td><td><span class="domain-research-type-badge">${escapeHtml(typeValue(item))}</span></td><td>${escapeHtml(nerPosition(item))}</td><td><span class="domain-kb-id">${escapeHtml(item.standard_kb_id || '内置知识库')}</span><br><span class="domain-mapping-status">${escapeHtml(item.mapping_status || '已映射')}</span></td>`}<td>${confidence(item.confidence)}</td><td><button class="${prefix}-detail-btn" data-viz-detail="${prefix}-detail-${index}">查看详情</button></td></tr><tr id="${prefix}-detail-${index}" class="${prefix}-detail-row" hidden><td class="${prefix}-detail-cell" colspan="${headers.length - 1}"><div class="${prefix}-detail-content"><div class="${prefix}-detail-label">${general ? '实体名称' : research ? '科研实体' : '专业实体'}</div><div class="${prefix}-detail-value">${renderTextWithMath(textValue(item))}</div><div class="${prefix}-detail-label">实体类型</div><div class="${prefix}-detail-value">${escapeHtml(typeValue(item))}</div><div class="${prefix}-detail-label">${research ? '句子位置' : '来源位置'}</div><div class="${prefix}-detail-value">${escapeHtml(nerPosition(item))}</div><div class="${prefix}-detail-label">关联上下文</div><div class="${prefix}-detail-value">${renderTextWithMath(item.context || '—')}</div><div class="${prefix}-detail-label">${general ? '规范实体编码' : research ? '实体编号' : '标准知识库ID'}</div><div class="${prefix}-detail-value">${escapeHtml(general ? item.canonical_entity_id || '未启用' : research ? idValue(item, index) || '未映射' : item.standard_kb_id || '未映射')}</div></div></td></tr>`).join('') || `<tr><td colspan="${headers.length}">未识别到达到阈值的实体。</td></tr>`}</tbody></table></div></div>
    <div class="${tabClass}-tab-panel" data-viz-panel="mapping" hidden><div class="distribution-table-wrap"><table class="distribution-table ${prefix}-mapping-table">${research ? '<colgroup><col style="width:18%"><col style="width:23%"><col style="width:27%"><col style="width:14%"><col style="width:18%"></colgroup>' : ''}<thead><tr>${general ? '<th>规范实体编码</th><th>中文规范名</th><th>英文规范名</th><th>缩写</th><th>其他别名</th><th>当前文本出现形式</th><th>实体类别</th><th>出现次数</th>' : research ? '<th>实体编号</th><th>中文标准词</th><th>英文标准词</th><th>实体类型</th><th>映射状态/置信度</th>' : '<th>标准知识库ID</th><th>领域/实体类型</th><th>标准名称</th><th>本体分类路径</th><th>别名</th><th>当前识别实体</th><th>映射状态/置信度</th>'}</tr></thead><tbody>${mappings.map((item, index) => general ? `<tr><td><span class="ner-code">${escapeHtml(item.canonical_entity_id || '—')}</span></td><td>${escapeHtml(item.canonical_names?.zh || '—')}</td><td>${escapeHtml(item.canonical_names?.en || '—')}</td><td>${escapeHtml(join(item.abbreviations))}</td><td>${escapeHtml(join([...(item.aliases?.zh || []), ...(item.aliases?.en || [])]))}</td><td>${renderTextWithMath(join(array(item.observed_mentions).map(x => x.text)))}</td><td>${escapeHtml(typeValue(item))}</td><td>${number(item.occurrence_count, array(item.observed_mentions).length)}</td></tr>` : research ? `<tr><td><span class="research-standard-id">${escapeHtml(idValue(item, index) || '—')}</span></td><td>${escapeHtml(item.standard_names?.zh || '—')}</td><td>${escapeHtml(item.standard_names?.en || '—')}</td><td>${escapeHtml(typeValue(item))}</td><td style="white-space:nowrap"><span class="research-mapping-status">${escapeHtml(mappingStatusLabel(item))}</span><span style="margin-left:8px">${confidence(item.mapping_confidence)}</span></td></tr>` : `<tr><td><span class="domain-kb-id">${escapeHtml(item.standard_kb_id || '—')}</span></td><td>${escapeHtml(item.domain_name || item.domain || '—')}<br>${escapeHtml(typeValue(item))}</td><td>${escapeHtml(item.standard_names?.zh || '—')}<br>${escapeHtml(item.standard_names?.en || '—')}</td><td>${escapeHtml(item.ontology_path || '—')}</td><td>${escapeHtml(join(item.aliases))}</td><td>${renderTextWithMath(join(array(item.observed_mentions).map(x => x.text)))}</td><td><span class="domain-mapping-status">${escapeHtml(mappingStatusLabel(item))}</span><br>${confidence(item.mapping_confidence)}</td></tr>`).join('') || `<tr><td colspan="${general ? 8 : research ? 5 : 7}">暂无映射结果。</td></tr>`}</tbody></table></div></div>
  </div>`
}
