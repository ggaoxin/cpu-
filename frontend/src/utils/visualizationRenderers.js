...
function renderNer(response, variant) {
  ...
  const cards = general
    ...
    : research
      ...
      : [['专业实体总数', entities.length], ['医学', entities.filter(item => (item.domain_name || item.domain) === '医学').length], ['化工', entities.filter(item => (item.domain_name || item.domain) === '化工').length], ['物理', entities.filter(item => (item.domain_name || item.domain) === '物理').length], ['已映射', entities.filter(item => item.standard_kb_id).length]]
  ...
  if (!research) {
    ...
    return `<div class="domain-research-result-root" data-viz-group>
      ...
      <div class="domain-research-tab-panel" data-viz-panel="entities"><div class="distribution-table-wrap"><table class="distribution-table domain-research-results-table"><colgroup><col style="width:11%"><col style="width:5%"><col style="width:14%"><col style="width:7%"><col style="width:7%"><col style="width:8%"><col style="width:22%"><col style="width:7%"><col style="width:7%"></colgroup><thead><tr>${domainHeaders.map(item => `<th>${item}</th>`).join('')}</tr></thead><tbody>${domainEntityRows || '<tr><td colspan="9">未识别到达到阈值的实体。</td></tr>'}</tbody></table></div></div>
