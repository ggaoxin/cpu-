...
function renderDeepCluster(response) {
  ...
  const describeCluster = clusterId => {
    ...
    const phrases = array(item?.representative_terms).slice(0, 2).join(' / ')
    return clusterId ? `${clusterId}${phrases ? ` · ${phrases}` : ''}` : '—'
  }
  ...
  const clusterCards = `<div class="deep-cluster-cards">${clusters.map(cluster => { const stats = object(cluster.feature_statistics); return `<div class="deep-cluster-card"><div class="deep-cluster-card-head"><div class="deep-cluster-card-title">${escapeHtml(cluster.cluster_id)}</div><div class="deep-cluster-card-size">${number(cluster.size)} 篇 · ${(number(cluster.ratio) * 100).toFixed(1)}%</div></div><div class="deep-cluster-term-row">${array(cluster.representative_terms).map(term => `<span class="deep-cluster-term">${renderTextWithMath(term)}</span>`).join('')}</div>${[['类内相似度', stats.intra_cluster_similarity], ['类间分离度', stats.inter_cluster_separation], ['语义密度', stats.semantic_density]].map(([label, value]) => `<div class="deep-cluster-stat-row"><span>${label}</span><i class="deep-cluster-stat-track"><b class="deep-cluster-stat-fill" style="width:${number(value) * 100}%"></b></i><strong>${fixed(value)}</strong></div>`).join('')}<div class="deep-cluster-doc-links"><b>代表文献：</b>${array(cluster.representative_documents).map(doc => `${escapeHtml(doc.document_id)} ${renderTextWithMath(doc.title)}`).join('；') || '—'}</div></div>` }).join('')}</div>`
  ...
  const trends = `<div class="deep-cluster-trend-insights"><div class="deep-cluster-insight"><b>上升类簇</b><span>${renderTextWithMath(describeCluster(trend.rising_cluster_id))}</span></div><div class="deep-cluster-insight"><b>新兴类簇</b><span>${renderTextWithMath(describeCluster(trend.emerging_cluster_id))}</span></div><div class="deep-cluster-insight"><b>稳定类簇</b><span>${renderTextWithMath(describeCluster(trend.stable_cluster_id))}</span></div></div><div class="deep-cluster-trend-card"><div class="deep-cluster-panel-title">类簇年度分布</div><div class="deep-cluster-year-header"><span>类簇编号 / 代表短语</span>${array(trend.years).map(year => `<span>${year}</span>`).join('')}</div>${array(trend.series).map((item, index) => `<div class="deep-cluster-trend-row"><div class="deep-cluster-trend-label"><i class="deep-cluster-legend-dot" style="display:inline-block;background:${clusterColors[index % clusterColors.length]};margin-right:6px"></i><b>${escapeHtml(item.cluster_id)}</b><small style="display:block;margin-left:18px;color:#7a889b">${renderTextWithMath(array(item.representative_terms).slice(0, 2).join(' / '))}</small></div>${array(item.yearly_counts).map(value => `<div class="deep-cluster-trend-cell"><span>${value}</span><i class="deep-cluster-trend-bar" style="height:${Math.max(3, number(value) / maxTrend * 58)}px;background:${clusterColors[index % clusterColors.length]}"></i></div>`).join('')}</div>`).join('')}</div><div class="deep-cluster-method-note"><b>趋势判断：</b>${renderTextWithMath(trend.summary || '')}</div>`
