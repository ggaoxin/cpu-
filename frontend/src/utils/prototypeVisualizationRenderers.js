...
function recordsOf(response) {
  ...
  return [{ index: 0, status: 'success', record_id: response?.meta?.record_id || data?.record_id || '', name: singleFileName || data?.document_title || data?.document?.title || '当前文献', payload: data }]
}
...
const clusterColors = ['#1f77b4','#ff7f0e','#2ca02c','#d62728','#9467bd','#8c564b','#e377c2','#7f7f7f','#bcbd22','#17becf','#aec7e8','#ffbb78','#98df8a','#ff9896','#c5b0d5','#c49c94','#f7b6d2','#c7c7c7','#dbdb8d','#9edae5']
const clusterShapes = [
  { id: 'circle',  svg: (f) => `<circle r="6" fill="${f}" opacity=".92">` },
  { id: 'square',  svg: (f) => `<rect x="-5" y="-5" width="10" height="10" fill="${f}" opacity=".92">` },
  { id: 'triUp',   svg: (f) => `<path d="M0,-7 L6,5 L-6,5 Z" fill="${f}" opacity=".92">` },
  { id: 'triDown', svg: (f) => `<path d="M0,7 L6,-5 L-6,-5 Z" fill="${f}" opacity=".92">` },
  { id: 'diamond', svg: (f) => `<path d="M0,-8 L5,0 L0,8 L-5,0 Z" fill="${f}" opacity=".92">` },
  { id: 'cross',   svg: (f) => `<path d="M-1.5,-6 L1.5,-6 L1.5,-1.5 L6,-1.5 L6,1.5 L1.5,1.5 L1.5,6 L-1.5,6 L-1.5,1.5 L-6,1.5 L-6,-1.5 L-1.5,-1.5 Z" fill="${f}" opacity=".92">` },
]
const clusterShapeAt = (i) => clusterShapes[i % clusterShapes.length]
const clusterShapeSvg = (i, fill) => clusterShapeAt(i).svg(fill)
...
function renderDeepCluster(response) {
  ...
  const clusterIdentity = item => {
    // 簇正式命名优先（LLM 内容归纳/锚定采纳的 topic_name）；锚定仅 suggested
    // （低置信候选，实测 borderline 错配率高——如地震动文献最近邻落"混凝土
    // 结构"类）时 category_distribution 只是候选分布，不作簇标识；无正式名
    // 回退锚定采纳名，再回退代表短语
    const official = String(item?.topic_name || '').trim()
  ...
  // 趋势 series 只有 cluster_id+年度计数；主题名按编号从 clusters 反查
  const clusterById = Object.fromEntries(clusters.map(c => [String(c.cluster_id), c]))
  const trendTopic = (clusterId) => {
    const cluster = clusterById[String(clusterId)]
    return cluster?.topic_name || (array(cluster?.representative_terms).slice(0, 2).join(' / ') || '')
  }
  const trends = `<div class="deep-cluster-trend-insights"><div class="deep-cluster-insight"><b>上升类簇</b><span>${renderTextWithMath(describeCluster(trend.rising_cluster_id))}</span></div><div class="deep-cluster-insight"><b>新兴类簇</b><span>${renderTextWithMath(describeCluster(trend.emerging_cluster_id))}</span></div><div class="deep-cluster-insight"><b>稳定类簇</b><span>${renderTextWithMath(describeCluster(trend.stable_cluster_id))}</span></div></div><div class="deep-cluster-trend-card"><div class="deep-cluster-panel-title">类簇年度分布</div><div class="deep-cluster-year-header"><span>类簇编号 · 主题</span>${array(trend.years).map(year => `<span>${year}</span>`).join('')}</div>${array(trend.series).map((item, index) => `<div class="deep-cluster-trend-row"><div class="deep-cluster-trend-label"><i class="deep-cluster-legend-dot" style="display:inline-block;background:${clusterColors[index % clusterColors.length]};margin-right:6px"></i><b>${escapeHtml(item.cluster_id)}</b><small style="display:block;margin-left:18px;color:#7a889b">${renderTextWithMath(trendTopic(item.cluster_id))}</small></div>${array(item.yearly_counts).map(value => `<div class="deep-cluster-trend-cell"><span>${value}</span><i class="deep-cluster-trend-bar" style="height:${Math.max(3, number(value) / maxTrend * 58)}px;background:${clusterColors[index % clusterColors.length]}"></i></div>`).join('')}</div>`).join('')}</div><div class="deep-cluster-method-note"><b>趋势判断：</b>${renderTextWithMath(trend.summary || '')}</div>`
  return `<div class="deep-cluster-result-root" data-viz-group><div class="deep-cluster-summary-grid"><div class="deep-cluster-summary-card"><div class="deep-cluster-summary-label">输入文献</div><div class="deep-cluster-summary-value">${data.input_summary?.document_count || 0}<span class="deep-cluster-summary-unit">篇</span></div></div><div class="deep-cluster-summary-card"><div class="deep-cluster-summary-label">句子特征</div><div class="deep-cluster-summary-value">${data.input_summary?.parsed_sentence_count || 0}<span class="deep-cluster-summary-unit">句</span></div></div><div class="deep-cluster-summary-card"><div class="deep-cluster-summary-label">形成类簇</div><div class="deep-cluster-summary-value">${quality.cluster_count || 0}<span class="deep-cluster-summary-unit">个</span></div></div><div class="deep-cluster-summary-card"><div class="deep-cluster-summary-label">簇内平均相似度</div><div class="deep-cluster-summary-value">${quality.intra_cluster_similarity != null ? fixed(quality.intra_cluster_similarity, 2) : '—'}</div></div><div class="deep-cluster-summary-card"><div class="deep-cluster-summary-label">聚类维度</div><div class="deep-cluster-summary-value deep-cluster-summary-value-text">${escapeHtml(data.cluster_dimension_name || '—')}</div></div></div><div class="deep-cluster-result-tabs"><button class="deep-cluster-result-tab active" type="button" data-viz-tab="overview">聚类结果与类簇特征统计</button><button class="deep-cluster-result-tab" type="button" data-viz-tab="assignments">文献归属</button><button class="deep-cluster-result-tab" type="button" data-viz-tab="trends">主题趋势分析结果</button></div><div class="deep-cluster-result-panel active" data-viz-panel="overview">${projection}${clusterCards}</div><div class="deep-cluster-result-panel" data-viz-panel="assignments" hidden>${assignments}</div><div class="deep-cluster-result-panel" data-viz-panel="trends" hidden>${trends}</div></div>`
}
...
function renderClusterLabelReview(response) {
  ...
  const labelRecord = recordsOf(response)[0] || {}
  const labelRecordId = String(labelRecord.record_id || response?.meta?.record_id || data?.record_id || '')
  const reviewCell = row => {
    const state = labelByClusterId.get(String(row.cluster_id)) || {}
    if (state.user_confirmed) {
      return `<span class="optimization-status passed">已人工确认${state.manual_label ? `（改为「${escapeHtml(state.recommended_label || state.label || '')}」）` : ''}</span>`
    }
    // 通过差异化检查的簇无需人工复核，只有待复核的才给确认/改标入口
    if (row.optimization_status === 'passed') return '无'
    const current = escapeHtml(String(row.recommended_label ?? ''))
    return `<span class="cluster-label-review-actions-v705"><button type="button" class="ghost-btn" data-viz-label-ok="${escapeHtml(String(row.cluster_id))}" data-viz-label-text="${current}">✓ 正确</button><button type="button" class="ghost-btn" data-viz-label-edit="${escapeHtml(String(row.cluster_id))}">✕ 修改</button></span><div class="cluster-label-review-edit-v705" data-viz-label-row="${escapeHtml(String(row.cluster_id))}" hidden><input class="input" data-viz-label-input="${escapeHtml(String(row.cluster_id))}" value="${current}" maxlength="40" placeholder="填写合适的类簇标签" /><button type="button" class="outline-btn" data-viz-label-submit="${escapeHtml(String(row.cluster_id))}">提交</button></div>`
  }
  // 优化数据存在但 items 为空 = 全部达标 → 空态提示；只有整个优化对象缺失才回退全量 labels
  const _hasOptObject = optimization && (optimization.clusters != null || optimization.items != null)
  const _optSource = array(optimization.clusters).length ? array(optimization.clusters) : array(optimization.items)
  const optimizedRows = (_optSource.length ? _optSource.map(it => ({...it, before_distinctiveness: it.before_distinctiveness, after_distinctiveness: it.after_distinctiveness, recommended_label: it.after_label || it.before_label || it.recommended_label, distinctiveness: it.distinctiveness ?? labels.find(l => l.cluster_id === it.cluster_id)?.distinctiveness, difference_explanation: it.reason || it.difference_explanation, optimization_status: it.threshold_passed ? "passed" : "review"})) : (_hasOptObject ? [] : labels)).map(item => ({
    cluster_id: item.cluster_id,
    recommended_label: item.recommended_label,
    distinctiveness: item.distinctiveness,
    before_distinctiveness: item.before_distinctiveness,
    after_distinctiveness: item.after_distinctiveness,
    difference_explanation: item.optimization_explanation || item.difference_explanation || labelByClusterId.get(String(item.cluster_id))?.difference_explanation || '已完成候选标签去重和差异度筛选，保留当前类簇中代表性与差异度更高的标签。',
...
function renderStructuredReviewSupplement(response) {
  ...
  // 按文献分组：点击哪个文献chip，就在该chip下方就地展开该文献的证据表
  const evidenceDocOrder = []
  const evidenceDocIndex = new Map()
  evidenceRows.forEach(item => {
    const docKey = String(item.document_id || '—')
    if (!evidenceDocIndex.has(docKey)) { evidenceDocIndex.set(docKey, []); evidenceDocOrder.push(docKey) }
    evidenceDocIndex.get(docKey).push(item)
  })
  const evidenceSourceHeading = resultPositionHeading(evidenceRows)
  const docEvidenceTable = docKey => {
    const docRows = evidenceDocIndex.get(docKey) || []
    // 单文献表：编号/题名两列整表同值 → rowspan 跨全部行合并，不逐行重复
    const rowsHtml = docRows.map((item, index) => `<tr>${index === 0 ? `<td class="v710-review-doc-cell" rowspan="${docRows.length}">${escapeHtml(docKey)}</td><td class="v710-review-doc-cell" rowspan="${docRows.length}">${renderTextWithMath(item.title || '—')}</td>` : ''}<td>${escapeHtml(positionLabel(item))}</td><td>${renderTextWithMath(item.evidence_excerpt)}</td></tr>`).join('') || '<tr><td colspan="4">未返回可定位的原始文献证据。</td></tr>'
    return `<div class="v710-review-table-wrap"><table class="v710-review-table"><colgroup><col style="width:9%"><col style="width:16%"><col style="width:12%"><col style="width:63%"></colgroup><thead><tr><th>文献编号</th><th>文献题名</th><th>${evidenceSourceHeading}</th><th>关键句段</th></tr></thead><tbody>${rowsHtml}</tbody></table></div>`
  }
  const evidenceStore = evidenceDocOrder.map(docKey => `<div data-review-doc-table="${escapeHtml(docKey)}">${docEvidenceTable(docKey)}</div>`).join('')
  ...
  return `<div class="v710-review-result" data-viz-group>
    ...
    <div class="v710-review-result-panel active" data-viz-panel="tree"><div class="v710-review-tree">${treeHtml}</div><div class="v710-review-evidence-store" hidden>${evidenceStore}</div></div>
